...
#define DEFINE_UNARY_OP_KERNEL_K1(OP_NAME, OP_EXPR)\
    template<typename T>\
    __global__ void OP_NAME##_kernel_rank1(const DeviceTensorView<const T> lhs, T value, DeviceTensorView<T> dst) {\
...
#define DEFINE_UNARY_OP_KERNEL_K2(OP_NAME, OP_EXPR)\
    template<typename T>\
    __global__ void OP_NAME##_kernel_rank2(const DeviceTensorView<const T> lhs, T value, DeviceTensorView<T> dst) {\
        size_t x = blockIdx.x * blockDim.x + threadIdx.x;\
        size_t y = blockIdx.y * blockDim.y + threadIdx.y;\
\
        if (y < lhs.shape[0] && x < lhs.shape[1])\
            dst(y, x) = OP_EXPR;\
    }

#define DEFINE_UNARY_OP_KERNEL_K3(OP_NAME, OP_EXPR)\
    template<typename T>\
    __global__ void OP_NAME##_kernel_rank3(const DeviceTensorView<const T> lhs, T value, DeviceTensorView<T> dst) {\
        size_t x = blockIdx.x * blockDim.x + threadIdx.x;\
        size_t y = blockIdx.y * blockDim.y + threadIdx.y;\
        size_t z = blockIdx.z * blockDim.z + threadIdx.z;\
\
        if (z < lhs.shape[0] && y < lhs.shape[1] && x < lhs.shape[2])\
            dst(z, y, x) = OP_EXPR;\
    }

#define DEFINE_UNARY_OP_KERNEL_K4(OP_NAME, OP_EXPR)\
    template<typename T>\
    __global__ void OP_NAME##_kernel_rank4(const DeviceTensorView<const T> lhs, T value, DeviceTensorView<T> dst) {\
...
#define DEFINE_UNARY_OP_KERNEL_ND(OP_NAME, OP_EXPR)\
    template<typename T>\
    __global__ void OP_NAME##_kernel_nd(const DeviceTensorView<const T> lhs, T value, DeviceTensorView<T> dst) {\
...
namespace om 
{
    ...
    DEFINE_UNARY_OP_LAUNCH_H(sub_k);
    DEFINE_UNARY_OP_KERNEL_H(sub_k);

    DEFINE_UNARY_OP_LAUNCH_H(mul_k);
    DEFINE_UNARY_OP_KERNEL_H(mul_k);

    DEFINE_UNARY_OP_LAUNCH_H(div_k);
    DEFINE_UNARY_OP_KERNEL_H(div_k);
}