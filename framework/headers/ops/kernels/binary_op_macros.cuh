...
#define DEFINE_BINARY_OP_KERNEL_AND_LAUNCH(OP_NAME, OP_EXPR)                                      \
    template<typename T>                                                                          \
    __global__ void OP_NAME##_kernel(MatView<T> lhs, MatView<T> rhs, MatView<T> dst) {            \
        ...
        if (r < dst.rows && c < dst.cols)                                                         \
            dst(r, c) = OP_EXPR;                                                                  \
                                                                                                   ...
                                                                                                   \
    template<typename T>                                                                          \
    void launch_##OP_NAME(MatView<T> lhs, MatView<T> rhs, MatView<T> dst) {                       \
...
#define DEFINE_BINARY_OP_KERNEL_AND_LAUNCH_FRW_DEC(OP_NAME)                                         \
    template void launch_##OP_NAME<float>(MatView<float> lhs, MatView<float> rhs, MatView<float> dst);  \
    template void launch_##OP_NAME<int>(MatView<int> lhs, MatView<int> rhs, MatView<int> dst);          \
    template void launch_##OP_NAME<char>(MatView<char> lhs, MatView<char> rhs, MatView<char> dst);
